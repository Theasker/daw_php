...
function ubicaryahoo($coordenadas) {
  ...
  // Indicamos también búsqueda global 
  // http://maps.googleapis.com/maps/api/geocode/json?latlng=41.671441,-0.897556&sensor=false
  $search = 'http://maps.googleapis.com/maps/api/geocode/json?latlng=' . $coordenadas['latitud'] . ',' . $coordenadas['longitud'] . '&language=es&sensor=false';
  //$xml = simplexml_load_file($search);
  $xml = json_decode(file_get_contents($search));
 ...
 */
  ...
  $respuesta->assign("cp", "value", (string) $xml->results[0]->address_components[6]->long_name);
